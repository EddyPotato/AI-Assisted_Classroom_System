...
import { AlertTriangle, ChevronRight, Gamepad2 } from 'lucide-react';

export default function SimulationPanel({ triggerEvent, eventLogs }) {
  // Sets the default state to hidden/minimized
  const [isOpen, setIsOpen] = useState(false);

  return (
    <>
      {/* INVISIBLE SPACER: Smoothly resizes the main dashboard when the panel opens/closes */}
      <div className={`transition-all duration-300 ease-in-out shrink-0 ${isOpen ? 'w-80' : 'w-0'}`} />

      {/* FLOATING TOGGLE BUTTON */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className={`fixed top-24 w-12 h-12 bg-blue-600 hover:bg-blue-700 text-white rounded-l-xl flex items-center justify-center shadow-lg transition-all duration-300 ease-in-out border-y border-l border-blue-700 z-40 group ${
      ...
      {/* SLIDING PANEL */}
      <aside 
        className={`fixed top-16 right-0 bottom-0 w-80 bg-white text-gray-700 flex flex-col border-l border-gray-200 shadow-2xl z-30 transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-5 border-b border-blue-100 bg-blue-50/50">
          <h3 className="text-lg font-black text-gray-800 flex items-center gap-2 tracking-tight">
            <Gamepad2 size={20} className="text-blue-600"/> SIMULATION PANEL
          </h3>
        </div>
