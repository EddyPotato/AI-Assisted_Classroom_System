...
export default function SchedulesTab() {
  ...
  const filteredSchedules = schedules.filter(s => {
    ...
    const matchesTime = timeFilter ? (s.time_Start || '').toUpperCase().includes(timeFilter) : true;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="p-5 border border-slate-200 bg-white rounded-2xl grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-4 shadow-sm">
        ...
        <div className="relative w-full">
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2">
            {timeFilter ? (
                <button onClick={() => setTimeFilter('')} className="text-slate-400 hover:text-rose-500 transition-colors p-1 rounded-md hover:bg-rose-50 bg-slate-50">
                  <X size={16} strokeWidth={2.5} />
                </button>
            ) : (
                <ChevronDown size={16} className="text-slate-400 pointer-events-none" />
