...
import { Edit2, Trash2, ArrowUpDown, ChevronUp, ChevronDown, Clock, MapPin, UserCircle, Filter, BookOpen } from 'lucide-react';
...
export default function SchedulesTab({
  ...
  const [typeFilter, setTypeFilter] = useState('All'); // NEW: Lec/Lab Filter

  const filteredSchedule = scheduleData.filter(sched => {
    const matchesSearch = sched.subject_Code.toLowerCase().includes(searchQuery.toLowerCase()) || 
    ...
    const matchesType = typeFilter === 'All' || sched.subject_Type === typeFilter;
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      {/* NEW: Sub-Toolbar for Lec/Lab Filter */}
      <div className="px-5 py-3 border-b border-slate-100 bg-white flex justify-between items-center">
         <span className="text-sm font-bold text-slate-500 flex items-center gap-2">
            <Filter size={16} className="text-indigo-500" /> Filter by Type
         </span>
         <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
            {['All', 'Lec', 'Lab'].map(type => (
               <button
                 key={type}
                 onClick={() => setTypeFilter(type)}
                 className={`px-4 py-1.5 rounded-lg font-bold text-sm transition-all ${typeFilter === type ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
               >
      ...
      <div className="overflow-x-auto w-full">
        {/* THE FIX: Fixed table width and explicit column sizes */}
        <table className="w-full text-left border-collapse min-w-250">
          <thead>
            <tr className="bg-white text-xs uppercase text-slate-500 font-black border-b-2 border-slate-100 cursor-pointer select-none">
              <th className="p-4 w-32 hover:bg-slate-50 transition-colors outline-none" onClick={() => onHandleSchedSort('subject_Code')}>
          ...
          <tbody className="divide-y divide-slate-50">
            {sortedSchedule.map((sched) => (
              <tr key={sched.schedule_ID} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="p-4 font-bold text-indigo-600 font-mono text-sm">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    {/* THE FIX: Added Professor Avatar + Click to Zoom logic */}
            ...
            {sortedSchedule.length === 0 && (
              <tr><td colSpan="6" className="p-12 text-center text-slate-500 font-bold">No subjects scheduled for this filter.</td></tr>
            )}
          </tbody>
        </table>
      </div>
