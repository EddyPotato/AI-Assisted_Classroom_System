...
export default function AccessLogEntry({ entry }) {
  ...
  return (
    <div className={`p-3 rounded-xl border shadow-sm ${isBypass ? 'bg-amber-50 border-amber-200' : isApproved ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span className={`text-[10px] font-black px-2.5 py-1 rounded-md text-white shadow-sm ${isBypass ? 'bg-amber-500' : isApproved ? 'bg-emerald-500' : 'bg-rose-500'}`}>
