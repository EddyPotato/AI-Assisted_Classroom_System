...
export default function VerificationPanel({ latestScan, cacheBuster }) {
  ...
  return (
    <div className={`w-full max-w-xl aspect-square lg:h-full lg:w-auto lg:max-h-full lg:max-w-full mx-auto border rounded-3xl sm:rounded-[2.5rem] p-4 sm:p-5 lg:p-7 shadow-sm transition-all duration-300 flex flex-col justify-between overflow-y-auto overflow-x-hidden ${isScanning ? 'bg-blue-50 border-blue-300 ring-4 ring-blue-500/20' : isMissing ? 'bg-amber-50 border-amber-300 ring-4 ring-amber-500/20' : isApproved ? 'bg-emerald-50 border-emerald-300 ring-4 ring-emerald-500/20' : 'bg-rose-50 border-rose-300 ring-4 ring-rose-500/20'}`}>
      ...
      {/* Status Badge */}
      <div className={`p-3 lg:p-4 rounded-xl sm:rounded-2xl font-black text-sm lg:text-base xl:text-lg flex items-center justify-center gap-2 sm:gap-3 w-full text-white shadow-md mt-auto tracking-widest shrink-0 ${isScanning ? 'bg-blue-600 animate-pulse' : isMissing ? 'bg-amber-500' : isApproved ? 'bg-emerald-600' : 'bg-rose-600'}`}>
        {isScanning ? <ScanLine className="animate-spin w-5 h-5 lg:w-6 lg:h-6 shrink-0" /> : isMissing ? <AlertCircle className="w-5 h-5 lg:w-6 lg:h-6 shrink-0" /> : isApproved ? <CheckCircle2 className="w-5 h-5 lg:w-6 lg:h-6 shrink-0" /> : <AlertCircle className="w-5 h-5 lg:w-6 lg:h-6 shrink-0" />}
